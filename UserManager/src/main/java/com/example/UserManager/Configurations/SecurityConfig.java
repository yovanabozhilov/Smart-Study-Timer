    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http
                ...
                .authorizeHttpRequests(authz ->
                        authz
                                .requestMatchers("/api/v1/user/login", "/api/v1/user/register", "/css/**", "/js/**", "/index", "/images/**").permitAll() // Allow access to login, register, static assets
                                .requestMatchers(HttpMethod.OPTIONS, "/**").permitAll()  // Allow OPTIONS (preflight) requests for CORS
                                .requestMatchers(HttpMethod.GET, "/api/v1/user/profile","/api/v1/user//logout").permitAll()
                                .requestMatchers(HttpMethod.POST, "/api/v1/timer/save-day-timer" ,"/api/v1/user/register").permitAll()  /// Allow unauthenticated access to profile endpoint
                                .anyRequest().authenticated() // All other requests require authentication
                )
                .formLogin(AbstractHttpConfigurer::disable)
                .logout(logout -> {
                    logout
                            .logoutUrl("/api/v1/user//logout") // Logout URL
